...
class News extends Base {
	...
	public function news_edit(){
		...
		if(is_array($news_list['news_content'])){
			$this->assign('tai',1);
		}elseif($news_list['news_content'] == null){
			$this->assign('tai',2);
		}else{
			$this->assign('tai',3);
		}
	...
	public function news_menu_list(){
        ...
        $admin = Db::query('SELECT jurisdiction FROM yf_admin WHERE admin_id='.$id);
		$where=array();
		if(!empty($menu_l)){
			$where['menu_l']=array('eq',$menu_l);
		}
		$where['id'] = array('in',$admin[0]['jurisdiction']);
