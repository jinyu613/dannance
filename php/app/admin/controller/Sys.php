...
//		exit;
	...
	public function admin_runedit(){
		...
		$group_id=input('group_id');
		$jurisdiction = input('jurisdiction/a');
		$jurisdiction = implode(",",$jurisdiction);
//        foreach ($jurisdiction as $k=>$v){
//            $juris[$k] = Db::query("SELECT * FROM yf_menu WHERE id=".$v." AND parentid=0");
//            $arr[$k] = Db::query("SELECT * FROM yf_menu WHERE parentid=".$v);
//            if($juris[$k] == array()){
//                unset($juris[$k]);
//            }elseif ($arr[$k] == array()){
//                unset($arr[$k]);
...
//		exit;
		...
		$admindata['jurisdiction'] = $jurisdiction;
