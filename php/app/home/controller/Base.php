...
class Base extends Common{
	...
	protected function _initialize(){
        ...
        $n['menu_l'] = 'zh-cn';
        $n['parentid'] = 0;
        $nav = Db::name('menu')->where($n)->field(['id','menu_name','menu_listtpl','menu_newstpl','menu_address'])->order('listorder')->select();
        foreach($nav as $key=>$navs){
            if($navs['menu_name'] === '网站首页'){
                $this->assign('id',$navs['id']);
                unset($nav[$key]);
            }
        }
        $footBottom = Db::name('options')->where('option_id',7)->value('option_value');
        $footBottom = json_decode($footBottom,true);
		$this->assign('footBottom',$footBottom);
        $footurl = Db::name('menu')->where(['menu_open'=>1,'parentid'=>46])->field('menu_name,menu_address')->select();
        $this->assign(['nav'=>$nav,'foot'=>$footurl]);
	}
	/**
	 * 检查用户登录
	 */
